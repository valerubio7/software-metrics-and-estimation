...
Este delta modifica la capacidad `historia` (US-06, issue #34). Agrega la operación de modificación `PUT /projects/{project_id}/stories/{story_id}`, un conjunto cerrado de estados, una estimación de horas nullable y la inmutabilidad de la identidad de la historia. Los requisitos de creación de US-05 siguen vigentes; solo cambia el requisito de inicialización, porque la historia registrada ahora también expone la estimación de horas, inicialmente ausente.

Convenciones de los escenarios: los identificadores técnicos (`title`, `description`, `priority`, `status`, `acceptance_criteria`, `estimated_hours`, `story_points`, `id`, `project_id`), las rutas, los códigos de error (`invalid_request`, `validation_failed`, `story_not_found`, `internal_error`) y los códigos HTTP se mantienen tal cual. Los escenarios de error `422` indican los campos en `fields`.
...
El sistema MUST admitir únicamente `PUT` sobre la ruta `/projects/{project_id}/stories/{story_id}` en esta operación. Cualquier otro método (por ejemplo `GET`, `POST`, `PATCH` o `DELETE`) MUST responder `405` y MUST NOT modificar ninguna historia. La lectura individual de una historia queda fuera de este cambio.
...
- GIVEN la ruta de colección `/projects/{project_id}/stories` con la creación disponible
- WHEN se solicita `GET`, `PUT` o `DELETE` sobre ella
